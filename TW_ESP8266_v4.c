 ...
 *      Author: Turjasu Pyne
 *      Addition: This version has the TCP Client functioanlity setups
 *
 *      Uses USCI A0
 */

#include "GenericUART/TW_GenericUART_UCAx.h"
#include <msp430.h>
#include <stdint.h>
#include <string.h>
#include <stdlib.h>
...
const  char              _MdmGETurl[] = "GET http://www.turjasuzworld.in/demo/api/getdevicewifi.php?did=TD1004 HTTP/1.1\r\nHost: Turjasu\r\nConnection: close\r\n\r\n";
char              _MdmSETurl[] = "GET http://www.turjasuzworld.in/demo/api/setdevicetemp.php?did=TD1004-1&temp=999.99&day=19&mon=07&year=2020&hh=15&mm=52&ss=47&signal=45 HTTP/1.1\r\nHost: Turjasu\r\nConnection: close\r\n\r\n";;
...
uint8_t ConfigureEspUART(float baudrate, uint8_t interrupt_polling)
{
    ...
    uint16_t  val2;
    volatile int val4 = 0;
     _ESP_PORT_SEL |= (_ESP_UART_RX + _ESP_UART_TX);                             // P3.4,5 = USCI_A2 TXD/RXD
     ...
     if(val2 < 16)
     {
                 ...
                 UCA2BR0 = val2 % 256;                              // (see User's Guide)
                 UCA2BR1 = val2 / 256;                              //
                 UCA2MCTL |= UCBRS_7;
                 val3 = (val - val2)*8;
                 val4 = (int)(val3);
                 ...
                 {
                    case 0:
                        UCA2MCTL |= UCBRS_0 + UCBRF_0;            // Modulation UCBRSx=1, UCBRFx=0
                        break;
                    case 1:
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        UCA2MCTL |= UCBRS_3 + UCBRF_0;            // Modulation UCBRSx=1, UCBRFx=0
                        break;
                    case 4:
                        UCA2MCTL |= UCBRS_4 + UCBRF_0;            // Modulation UCBRSx=1, UCBRFx=0
                        break;
                    case 5:
                        UCA2MCTL |= UCBRS_5 + UCBRF_0;            // Modulation UCBRSx=1, UCBRFx=0
                        break;
                    case 6:
                        UCA2MCTL |= UCBRS_6 + UCBRF_0;            // Modulation UCBRSx=1, UCBRFx=0
                        break;
                    case 7:
                        UCA2MCTL |= UCBRS_7 + UCBRF_0;            // Modulation UCBRSx=1, UCBRFx=0
                        break;
                    default:
                        break;
                }
     ...
     {
         UCA2MCTL |= UCOS16;
         UCA2BR0 = ((uint16_t)(val/16))%256;
         UCA2BR1 = ((uint16_t)(val/16))/256;
     ...
     switch (interrupt_polling)
     {
        case 1:
            ...
            break;
        case 2:
             UCA2IE &= ~(UCRXIE + UCTXIE);                         // Enable USCI_A2 RX interrupt
             break;
        default:
            break;
    }
...
//    uint8_t indx=0;
        ...
        while(*data)
        {
            ...
            UCA2TXBUF = *data;                  // TX -> RXed character
                SendCharToUCA3(*data);
            data++;
        }
...
esp8266StateMachines    _ReadEspRSSI(char* apName, char* rssiHolder)
{
    ...
    do
    {
        ...
        res = mdmReplySearch((const char *)_MdmBuffer, (const char *)apName); // Checking for the OK will skip or loose the RSSI details which is sent slowly from ESP01
        if(res == NULL)
        {
            return _E8266_RSSI_RCV_TIMEOUT;
        }
...
esp8266StateMachines    MdmMakeReady(espOperCommand OperCmd, esp8266StateMachines currState, char* _getUrlPtr, char* _servUrlPtr, char* _udpServUrlPtr,char* _setUrlPtr,int _udpServPort, int _udpDataLen) // currState will be verified everytime
{
    ...
    static int  temp_count=0, waitCount = 0;
    ...
    switch (OperCmd)
    {
            case _Esp_Full_Init:
                ...
                if((RetSts == _E8266_PWR_UP)||(RetSts == _E8266_PWR_RESTART))
                {
                    ...
                    if (mdmReplySearch((const char*)_MdmBuffer, "OK")==NULL) RetSts = _E8266_PWR_UP_FAILURE;
                ...
                else if(RetSts == _E8266_PWR_DN)
                {
                    _ESP_RST_PORT_OUT &= ~_ESP_RST;
                    SendDataToESP("AT\r\n");
                    if (mdmReplySearch((const char*)_MdmBuffer, "OK")==NULL) RetSts = _E8266_PWR_OFF;
        ...
        case _Esp_Connect_WIFI:
             ...
             if((RetSts == _E8266_PWR_UP_SUCCESS)||(RetSts == _E8266_RST_SUCCESS)||(RetSts == _E8266_DeECHO_SUCCESS))
             {
                 ...
                 do
                 {
                     ...
                     SendDataToESP("AT+CWMODE=3\r\n"); // station mode set :: AT+CWMODE=3 for new ESP, AT+CWMODE_CUR=3 for old ESPs
                     //_delay_cycles(SystemFreq*3);
/*
                     res = NULL;
                     do
...
*/
                     ...
                     if(*res=='O')
                     {
                         ...
                         // SET HOSTNAME //
                          SendDataToESP("AT+CWHOSTNAME=\"ESP_F7E8AE\"\r\n");
                 ...
                 if(RetSts == _E8266_STN_MODE_SUCCESS)
                              {
                                  ...
                                  do
                                  {
                                      SendDataToESP("AT+CWJAP=\"");//:: AT+CWJAP for new ESP, AT+CWJAP_CUR for old ESPs
...
//                                              res = NULL;
//                                              do //NO MORE SUPPORTED IN NEW ESP01s
//                                              {
...
//                                              } while (res == NULL); // Blocking Call
                 ...
                 if(RetSts == _E8266_WIFI_RCV_IP_SUCCESS)
                 {
                     ...
                     do
                     {
                         ...
                         waitCount = 5;
                         SendDataToESP("AT+PING=\"www.turjasuzworld.in\"\r\n");
                         _delay_cycles(SystemFreq*5);
        ...
        case _Esp_CreateandConnect_TCP_Client:

            RetryTimeout1 = 5;
            do {
                if(_servUrlPtr == NULL){
                    RetSts =  _E8266_CIPSTART_SRVR_PTR_NULL;
                    break;
                }
                SendDataToUCA3("__case _Esp_Create_TCP_Client in MdmMakeReady__ Retry:");
                SendCharToUCA3(RetryTimeout1 + 48);
                // Connect to TCP Server
                SendDataToESP("AT+CIPSTART=\"TCP\",\"");
                SendDataToESP(_servUrlPtr);
                SendDataToESP("\",");
                if (_udpServPort < _TCP_PORT_MAX){
                    if((_udpServPort/10000)>0){
                        SendCharToESP((_udpServPort/10000)+48);
                        SendCharToESP(((_udpServPort%10000)/1000)+48);
                        SendCharToESP(((_udpServPort%1000)/100)+48);
                        SendCharToESP(((_udpServPort%100)/10)+48);
                        SendCharToESP((_udpServPort%10)+48);
                    }
                    else if ((_udpServPort/1000)>0){
                        SendCharToESP((_udpServPort/1000)+48);
                        SendCharToESP(((_udpServPort%1000)/100)+48);
                        SendCharToESP(((_udpServPort%100)/10)+48);
                        SendCharToESP((_udpServPort%10)+48);
                    }
                    else if ((_udpServPort/100)>0){
                    ...
                    SendDataToUCA3("TCP Port value Out of Bound > 65536");
                    RetSts = _E8266_TCP_PORT_VAL_INVALID;
                }
                SendDataToESP("\r\n");
                _delay_cycles(SystemFreq*3);// ==> remove this later
                ...
                {
                    if (NULL != mdmReplySearch(_MdmBuffer, "OK")){
                        RetSts =  _E8266_TCP_CONNECT_SRVR_SUCCESS;
                    }
                    else{
                        RetSts =  _E8266_TCP_CONNECT_SRVR_INVALID_STATE;
                        SendDataToUCA3("mdmReplySearch failed to find CONNECT OK");
                    }
                }
                RetryTimeout1--;

            } while ((RetryTimeout1 >0)&&(RetSts != _E8266_TCP_CONNECT_SRVR_SUCCESS));
...
/*
 * This part has been added in the v4
 * Handles the TCP Client functionality that can be used in TCP socket comms
 * @param  _state : Accepts the current state in which the ESP8266 module is currently in
 * @return the state in which the module remains after the creation of the client.
 *          This function can be called recursively. Requires Stack check in RT
 */
esp8266StateMachines    _createTcpClient(esp8266StateMachines _state, const char* TcpServerIPAddr,uint16_t port )
{
    volatile  esp8266StateMachines tempState = _state;

    switch (tempState) {
        case _E8266_SEND_OK_RECVD:
        case _E8266_PING_FAIL:
        case _E8266_PING_TIMEOUT:
            tempState = MdmMakeReady(_Esp_CreateandConnect_TCP_Client, tempState, NULL, (char*)TcpServerIPAddr, NULL, NULL, port, NULL);
            if(tempState !=_E8266_TCP_CONNECT_SRVR_SUCCESS){
                tempState = _E8266_TCP_CREATECLIENT_FAILED;
            }
...
/*
 * This Fxn checks if the device is a virgin device. If it has never been connected
 * to any SSID then it has to go through the first config.
 *
 * @param:  ns - set the state for start up of module. Preferably keep it as _BRN_START if starting connection
 *
 * @param   wssid is the wifi ssid that needs to be hard coded if LCD is absent
 *
 * @param   wpswd  is the wifi password that needs to be hard coded if LCD is absent
 *
 * @param   wifiFailCntMax is the maximum number of retires needed before connectivity is given up
 *
 * @ret     returns the state, either
 *
 */
nStates                _genericProvisionAndConnectEsp01(nStates ns,const char * wssid,  const char * wpswd, const int wifiFailCntMax, bool getFuncChkReqd)
{
    volatile nStates nDeviceStates = _BRN_NON_INITIALIZED;
    volatile esp8266StateMachines    _noLCDModuleState = _UNKNOWN;
    nDeviceStates = ns;//_BRN_START;
    int _wifiConnFailureTimes = 0;
    //WriteTo24CxxEEPROMMultiBytes_InterruptMode(0xae, 64, (14*32), 32, "123456789012345678901234567890");// ==> only for debug, remove!!!
    _delay_cycles(SystemFreq);
    // restore or use default values for non-lcd versions !
    if(wssid == NULL) return _BRN_INVALID_WIFI_SSID;
    if(wpswd == NULL) return _BRN_INVALID_WIFI_PSWD;
    if(NULL == strcpy(WifiParams._wifiSSID, wssid)) return _BRN_UNABLE_TO_SET_WIFI_CREDS;
    if(NULL == strcpy(WifiParams._wifiPSWD, wpswd)) return _BRN_UNABLE_TO_SET_WIFI_CREDS;

    while(_wifiConnFailureTimes < wifiFailCntMax)
    {
        if(getFuncChkReqd)
        {
            _noLCDModuleState = _provision_Connect_GETcall_ESP(_E8266_PWR_UP,getFuncChkReqd,(char*)_MdmGETurl, (char*)_serverURL);
            _noLCDModuleState = _provision_Connect_GETcall_ESP(_noLCDModuleState,getFuncChkReqd,(char*)_MdmGETurl, (char*)_serverURL);
            if(_noLCDModuleState == _E8266_SEND_OK_RECVD)
            {
                // inform on LED / LCD that connection has happened WIFI_CN_LED_ON();
                _wifiConnFailureTimes = 100; // 100 > 10 just to ensure that the above while doesn't re-loop if connected
                nDeviceStates = _BRN_RDY_GET;
                SendDataToUCA3("__Wifi Connected successfully__\r\n");
            }
            else
            {
                // Since the wifi could not be connected in last step,
                //Forcefully make device a Virgin Device, so that
                //the SSID and PWD may be entered again by user

                _wifiConnFailureTimes++;
                SendDataToUCA3("__Wifi Connection failure, retrying... Number:\r\n");
                SendCharToUCA3((_wifiConnFailureTimes/10)+48);
                SendCharToUCA3((_wifiConnFailureTimes%10)+48);
                SendDataToUCA3("\r\n");
            }
        }
        else
        {
            _noLCDModuleState = _provision_Connect_GETcall_ESP(_E8266_PWR_UP,getFuncChkReqd,(char*)_MdmGETurl, (char*)_serverURL);
            _noLCDModuleState = _provision_Connect_GETcall_ESP(_noLCDModuleState,getFuncChkReqd,(char*)_MdmGETurl, (char*)_serverURL);
            if( _noLCDModuleState == _E8266_PING_SUCCESS)
            {
                // inform on LED / LCD that connection has happened WIFI_CN_LED_ON();
                _wifiConnFailureTimes = 100; // 100 > 10 just to ensure that the above while doesn't re-loop if connected
                nDeviceStates = _BRN_RDY;
                SendDataToUCA3("__Wifi Connected successfully without get service checking__\r\n");
            }
            else
            {
                // Since the wifi could not be connected in last step,
        ...
        _delay_cycles(SystemFreq); //just wait 1 sec before retrying!!
    }
    if(_wifiConnFailureTimes == wifiFailCntMax)
    {
        SendDataToUCA3("__Unable to connect to Wifi... FORCED VIRGIN CONVERSION !!__\r\n");
        nDeviceStates = _BRN_WIFI_CONNECT_RETRY_TIMEOUT_ERR;
        // ==> reseting management shud be handled else whr //WDTCTL = 0x6A00;       // As of now resets the device so that the device enters the virgin device loop to take the SSID & PWD
    }
return nDeviceStates;
...
#endif
{
  ...
  case 2:                                   // Vector 2 - RXIFG
      _MdmBuffer[_MdmBuffCnt] = UCA2RXBUF;                  // TX -> RXed character
